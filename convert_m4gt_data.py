"""
Convert m4gt OOD split data + logprobs into ghostbuster format.
...
data_folder = "data/m4gt/OOD/"
logits_file = "data/m4gt.logits.json"
prefix = "m4gt"
...
for d in os.listdir(data_folder):
    ...
    human_folder = f"data/{prefix}_{d}/train/human"
    human_logprob_folder = os.path.join(human_folder, "logprobs")
    gpt_folder = f"data/{prefix}_{d}/train/gpt"
...
for d in os.listdir(data_folder):
    ...
    human_folder = f"data/{prefix}_{d}/test/human"
    human_logprob_folder = os.path.join(human_folder, "logprobs")
    gpt_folder = f"data/{prefix}_{d}/test/gpt"
    gpt_logprob_folder = os.path.join(gpt_folder, "logprobs")
    os.makedirs(human_logprob_folder, exist_ok=True)
    os.makedirs(gpt_logprob_folder, exist_ok=True)
